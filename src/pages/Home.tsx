...
function AnimatedCounter({ target, suffix = '' }: { target: number | string; suffix?: string }) {
  ...
  const num = typeof target === 'number' ? target : parseInt(target as string);
...
/* ─────────────────────────────────────────────────────────────
   HeroCrystals — pure CSS/Framer Motion, zero WebGL/Three.js
   Works on ALL devices: desktop, tablet, low-end Android/iOS
   ───────────────────────────────────────────────────────────── */
function HeroCrystals({ darkMode }: { darkMode: boolean }) {
  // Stable random values — generated once via ref so rerenders don't reshuffle
  ...
  const rings = [
    ...
    { cx: '84%', cy: '16%', size: 150, delay: 2, duration: 8   },
  ...
  return (
    ...
    >
      ...
      {/* ── Extra: diagonal grid lines (very subtle, desktop only) ── */}
      <svg
        style={{
          position: 'absolute',
          inset: 0,
  ...
  return (
    <>
      ...
      <div style={{ background: colors.background, color: colors.text, minHeight: '100vh', overflowX: 'hidden', transition: 'background-color 0.3s, color 0.3s', position: 'relative' }}>
        ...
        >
          {/* All-device animated background */}
